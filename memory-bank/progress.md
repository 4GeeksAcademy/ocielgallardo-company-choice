...
- Telemetry technical report: **complete** on `feature/telemetry-technical-report` — Pandas pipeline + `GET /telemetry/report` (incl. `auth_failure_rate`) + backoffice `/telemetry` tables.
...
- Pipeline: `services/app/domain/telemetry_analysis.py` — pure functions `events_per_day`, `error_rate_by_type`, `avg_latency_by_path`, `auth_failure_rate` + `build_report` (SQL window → Pandas UTC timestamps → groupby/agg → `list[dict]`).
- Endpoint: `GET /telemetry/report` — optional `start_date`/`end_date` (default last 7 days UTC); in-memory `telemetry_report_cache` TTL 60s; response `{ period, metrics }` including `auth_failure_rate`.
- Backoffice: `/telemetry` technical dashboard (HTML tables) via `telemetryApi.ts` + nav group Telemetry; no chart library.
- Out of scope this pass: PR title packaging only remaining.
