...
type FormStatusVariant = "success" | "error" | "info";

const VARIANT_STYLES: Record<FormStatusVariant, string> = {
  success:
...
export function FormStatusMessage({
  ...
  return (
    <div
      className={`rounded-lg border p-4 text-sm ${VARIANT_STYLES[variant]} ${className}`.trim()}
